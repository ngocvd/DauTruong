...
namespace zap
{
    ...
    public partial class MAIN_Form : Form
    {
        ...
        int lastRecord = 0;
        string[] fileMovies = null;
        string[] fileSrcs = null;
        //public Hashtable LedList = new Hashtable();
        List<MyTime> LedList = new List<MyTime>();
        ...
        public MAIN_Form(string MediaFile)
        {
            ...
            Sound_Track.Value = 90;
            //Movie_Timer.Interval = stepTime * 20;
        ...
        private void CheckMovieName(string MediaFile)
        {
            if (MediaFile.Length != 0)
            {
                ...
                if (MediaFile.EndsWith(".mkv")) DoIt = true;
                if (MediaFile.EndsWith(".mp4")) DoIt = true;
                if (MediaFile.EndsWith(".flv")) DoIt = true;
        ...
        public void LoadSrc(string fileSrc)
        {
            string fileName = CurFolder + fileSrc;// + Properties.Settings.Default.SrcFile;
            if (!File.Exists(fileName))
            {
                MessageBox.Show("Không có file: "+ fileName);
                return;
            }
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    foreach (Match match in regex01.Matches(line))
                    {
                        if (match.Groups.Count > 1)
                        {
                            ...
                            {
                                MyTime t = new MyTime();
                                t.TimeToDouble = RevertCalculateTime(row[0]);
                                t.TimeString = row[0];
                                ...
                                foreach (Match m1 in regex1.Matches(row[1]))
                                {
                                    if (m1.Groups.Count > 1)
                                    {
                                        byte slaveId = Convert.ToByte(m1.Groups[1].Value);
                                        string s = "";
                                        string ss = m1.Groups[2].Value;
                                        foreach (Match m2 in  regex2.Matches(ss))
                                        {
                                            if(m2.Groups.Count > 1)
                                            s += Convert.ToString(Convert.ToByte(m2.Groups[1].Value, 2), 16);
                                        }
                                        if ((s.Length % 4) != 0)
                                            s += "00";
                                        t.IDs[slaveId - 1] = s;
        ...
        public int GetLedEqual(int lr, double Time)
        {
            //var compareDate = RevertCalculateTime(Time);
            //or whatever date you want
            int l = 0;
            for(int i = lr; i<LedList.Count; i++)
            {
                if (LedList[i].TimeToDouble <= Time)
                {
                    l = i;
                }
                else break;
        ...
        private void Movie_Timer_Tick(object sender, EventArgs e)
        {
            ...
            if (Movie_Timer_Enabled)
            {
                ...
                //string t = CalculateTime(MovieCurrentPosition);
                //string s = GetLedEqual(t);
                //if (s!=null)
                //{
                //    if (!mbSystem.IsDead)
                //        mbSystem.LedTurnOn(s);
                //}
        ...
        {
            mbSystem = new MyModbus();
            MyTimer.Stop();
            string s = " ";
            if(mbSystem.Checked == false)
                s=mbSystem.CheckId();
            DialogResult dialogResult = DialogResult.Yes;

            if (s != "")
                dialogResult = MessageBox.Show("Bạn tiếp tục không?", "Các ID không kết nối: " + s, MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.No)
            {
                if(mbSystem.IsDead==false)
                {
                    mbSystem.Dispose();
                    //mbSystem = null;
                }
...
//Size MovieMinimumIdealSize = Movie.MinimumIdealSize;
        ...
        //private void soạnFileToolStripMenuItem_Click(object sender, EventArgs e)
        //{
        //    // Create a new instance of the Form2 class
        //    Password settingsForm = new Password();

        //    // Show the settings form
        //    settingsForm.Show();
        //}

        private void soanFileXaBanToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if(mbSystem.IsDead==false)
                mbSystem.Dispose();
            //mbSystem = null;
        ...
        {
            MyTimer.Stop();
            double MovieCurrentPosition = Movie.CurrentPosition;
            //string t = CalculateTime(MovieCurrentPosition);
            int l = GetLedEqual(lastRecord, MovieCurrentPosition);
            if (l != lastRecord)
            {
                if (!mbSystem.IsDead)
                    mbSystem.LedTurnOn(LedList[l]);
                lastRecord = l;
            }
            MyTimer.Start();
        }
        ...
        {
            //mbSystem = new MyModbus();
            //string s = mbSystem.CheckId();
            //static System.Windows.Forms.Timer myTimer = new System.Windows.Forms.Timer();
            fileMovies = Properties.Settings.Default.VideoFileLang.Split(';');
            fileSrcs = Properties.Settings.Default.SrcFileLang.Split(';');
            MyTimer.Elapsed += new  System.Timers.ElapsedEventHandler
                (TimerEventProcessor);

            // Sets the timer interval to 5 seconds.
            MyTimer.Interval = stepTime * 10;
            //MyTimer.Start();

            LoadSrc(Properties.Settings.Default.SrcFile);
            PlayThisMovie(CurFolder + Properties.Settings.Default.VideoFile);
            //    BTN_Pause_Click(null, null);
        ...
        {
            if(fileMovies.Length>0 && fileSrcs.Length > 0)
            {
                mbSystem.Dispose();
                mbSystem = null;
                Movie = null;
                LoadSrc(fileSrcs[0]);
                PlayThisMovie(CurFolder + fileMovies[0]);
        ...
        {
            if (fileMovies.Length > 1 && fileSrcs.Length > 1)
            {
                mbSystem.Dispose();
                mbSystem = null;
                Movie=null;
                LoadSrc(fileSrcs[1]);
                PlayThisMovie(CurFolder + fileMovies[1]);
                
            }
        }

        private void tiengPhapToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (fileMovies.Length > 2 && fileSrcs.Length > 2)
            {
                mbSystem.Dispose();
                mbSystem = null;
                Movie = null;
                LoadSrc(fileSrcs[2]);
                PlayThisMovie(CurFolder + fileMovies[2]);
                //BTN_Play_Click(null, null);
            }
        }
